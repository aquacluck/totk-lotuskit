        ...
        void debugLog() {
            ...
            for (size_t i = 0; i < trace_size; ++i) {
                ...
                if (info.nearest_sym_name[0] == '\0') {
                    nn::util::SNPrintf(buf, sizeof(buf), "%s+%08x", info.module_name, info.offset);
                } else {
                    nn::util::SNPrintf(buf, sizeof(buf), "%s+%08x (%s+%08x)", info.module_name, info.offset, info.nearest_sym_name, info.nearest_sym_offset);
                }
