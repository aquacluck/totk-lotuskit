...
# ty Kito Cheng: https://kitoslab-eng.blogspot.com/2012/10/armv8-aarch64-instruction-encoding.html
kito_cheng_src_text = """\
...
class Encoding:
    ...
    def __init__(self, src_line):
        ...
        # 32 chars representing inst bits from left side
        self.bits_str = self.src_line.split("  -  ")[0].replace(' ', '')
        # mnemonic tokens from right side
        self.mnm_parts = self.src_line.split("  -  ")[1].split(' ')
        # unique letters in left side bit encoding, sometimes referenced on right side
        self.bits_argkeys = set(self.bits_str) - set("01")
        self.bits_sum_per_argkey = self._bits_sum_per_argkey()

    def __repr__(self):
        return f'Encoding("{self.src_line}")'

    def _bits_sum_per_argkey(self):
        out = defaultdict(int)
        char_exp = defaultdict(int)
        for i in range(-1, -33, -1): # -1 to -32 inclusive (we start from lsb source, rtl)
            char = self.bits_str[i]
            if char == '0' or char == '1':
                continue
            # each time we see the same char, bump its next bit
            out[char] += 2**char_exp[char]
            char_exp[char] += 1
        return out
    ...
    def cpp_is_match(self, identifier="inst"):
        return f"""(({identifier} & {self.u32_always}) == {self.u32_always} && (~{identifier} & {self.u32_never}) == {self.u32_never})"""

encodings = [Encoding(l) for l in kito_cheng_src_text.splitlines()]
...
for enc in encodings:
    mnm_trash = f'{enc.mnm_parts[0]:<7} {", ".join(enc.mnm_parts[1:])}'
    cpp_gen_out.write(f"""        if ({enc.cpp_is_match()}) {{ // {mnm_trash}
            strcpy(output, "{mnm_trash}");
            return;
        }}\n""")
