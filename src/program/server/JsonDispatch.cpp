...
namespace lotuskit::server {
    ...
    void JsonDispatch::dispatchText(const char* jsonText) {
        ...
        if (jsonPayload.contains("execScriptString")) {
            auto moduleName = jsonPayload.contains("execScriptModule") ? (jsonPayload["execScriptModule"].template get<String>()) : "web_anon.as";
            auto scriptText = jsonPayload["execScriptString"].template get<String>();
            bool blockOnDebugPause = jsonPayload.contains("blockOnDebugPause") ? (jsonPayload["blockOnDebugPause"].template get<bool>()) : true;
            lotuskit::script::schedule::tas::pushExecTextModule(moduleName, moduleName, scriptText, "void main()", false, blockOnDebugPause); // defer execution to main loop

        } else if (jsonPayload.contains("persistFileString") && jsonPayload.contains("persistFileName")) {
            auto src = jsonPayload["persistFileString"].template get<String>();
            auto path = jsonPayload["persistFileName"].template get<String>();
            lotuskit::util::fs::writeTextFile(src.c_str(), path);
            ...
            auto path = jsonPayload["persistFileName"].template get<String>();
            lotuskit::util::fs::writeFile(src.c_str(), src.size(), path);
            ...
            lotuskit::util::fs::writeFileChunked(&chunkedUploadFd, src.c_str(), src.size(), path, chunkOp, chunkOffset);
        }
    }
