...
- Experimental or potentially disruptive patches are kept in `ips_patches/runtime_patches` and may be loaded on demand like `sys::patchInstall("lotuskit-playercamera-unlock");`. `runtime_patches` is installed alongside `romfs`/`exefs`.
