...
private:
    ...
    // this should be sead::FixedTreeMap<DictID, ZSTD_DDict*, 32> but that has an extra pointer to
    // the map at the end of each node... seems there are probably two versions of the map since
    // some other structs maintain that extra pointer
    void* mRootNode;
    FreeList mFreeList;
    int mCount;
    int mMax;
